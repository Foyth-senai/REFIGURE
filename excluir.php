...
<div class="informacao mt-5">
    <form class="area" action="excluir.php" method="post">
        ...
        <input maxlength="14" id="cpf" onkeyup="ValidarCPF();" placeholder="Cpf que deseja excluir" name="cpf_alterar" class="input-style" type="text">
        </div>
        <br> <br>
        <div class="botoesalterar">
        <button class="btn1" type="submit" name="excluir"><b>Excluir</b></button>
        </div>
    </form>
    <?php
    ...
    while ($linhas = $comando->fetch() )
    {
       ...
       if(isset($_POST["excluir"]) )
       {
          $cpf_alterar = $_POST["cpf_alterar"];
           $comando = $pdo->prepare("DELETE FROM cadastro where cpf_cliente = '$cpf_alterar'");
           $resultado = $comando->execute();
