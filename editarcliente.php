...
<div class="informacao mt-5">
    <form class="area" action="editar.php" method="post">
        <div class="minhaconta"><h1 ><ion-icon class="icone" name="person-circle-outline"></ion-icon><b>Editar Informações do cliente</b></h1></div>
        <h2 class="fs-4 mt-4 d-flex justify-content-center"><b>Insira o seu CPF e altere sua senha</b></h2>
        <div class="informacoes">
        <input maxlength="14" id="cpf" onkeyup="ValidarCPF();" placeholder="Insira CPF do cliente" name="cpf_alterar" class="input-style" type="text">
    ...
    <?php
       if(isset($_POST["editar"]) )
       {
          ...
          $comando = $pdo->prepare("SELECT * FROM cadastro where cpf_cliente='$cpf_alterar'");
          $resultado = $comando->execute();
          if($nome_novo == ""){
            while ($linhas = $comando->fetch() )
            {
            $nome_novo = $linhas["nome_cliente"]; // Nome da coluna XAMPP
            }
          }
          if($email_novo == ""){
            while ($linhas = $comando->fetch() )
            {
            $email_novo = $linhas["email_cliente"]; // Nome da coluna XAMPP
            }
          }
          if($telefone_novo == ""){
            while ($linhas = $comando->fetch() )
            {
            $telefone_novo = $linhas["celular_cliente"]; // Nome da coluna XAMPP
            }
          }
